    ...
    switch (format[0]) {
      case 'p':
        ...
        if (rna_plot_type == 4) {
            ...
            puzzler->checkAncestorIntersections = (args_info.ignoreAncestorIntersections_flag == 0);
            puzzler->checkSiblingIntersections = (args_info.ignoreSiblingIntersections_flag == 0);
            puzzler->checkExteriorIntersections = (args_info.ignoreExteriorIntersections_flag == 0);
            puzzler->allowFlipping = (args_info.allowFlipping_flag != 0);
            puzzler->optimize = (args_info.noOptimization_flag == 0);

            (void)vrna_file_PS_rnaplot_a_puzzler(rec_sequence, structure, ffname, pre, post, &md, puzzler);

            puzzler->filename = NULL;
            destroyPuzzlerOptions(puzzler);
        } else {
            (void)vrna_file_PS_rnaplot_a_puzzler(rec_sequence, structure, ffname, pre, post, &md, NULL);
        }
